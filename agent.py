...
The module selects the minimum required tools, executes them, and returns the
exact shared output schema. User-facing messages are Arabic, while function
names and schema keys remain English for stable integration.
"""
...
from tools import SUPPORTED_DESTINATIONS, calculate_budget, create_itinerary
...
_AGENT_SYSTEM_PROMPT = """أنت وكيل التوجيه في تطبيق Saudi Explorer AI.
مهمتك اختيار أقل عدد من الأدوات اللازمة لتنفيذ طلب المستخدم.

الأدوات المتاحة فقط:
- create_itinerary: لإنشاء خطة رحلة أو جدول يومي أو برنامج سياحي.
...
- لا تختر أي أداة غير الأدوات المسجلة.
- أعد النتيجة وفق مخطط القرار المنظم فقط.
- يجب أن تبقى قيم action وtools_to_use بالإنجليزية كما هي في المخطط.
"""

_FINAL_ANSWER_SYSTEM_PROMPT = """أنت مساعد سفر سعودي باسم Saudi Explorer AI.
...
def decide_tools(
    ...
    or unavailable, the function returns a deterministic local decision.
    """
...
def _fallback_tool_answer(tools_used: list[str]) -> str:
    if tools_used == ["create_itinerary"]:
        return "تم إنشاء خطة الرحلة اعتمادًا على الأماكن المسترجعة من قاعدة المعرفة."
...
def run_agent(
    ...
    Args:
        user_query: User travel question, normally written in Arabic.
