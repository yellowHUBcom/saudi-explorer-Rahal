...
class _AgentDecision(BaseModel):
    ...
    def validate_action_matches_tools(self) -> "_AgentDecision":
        """Reject contradictory Agent decisions before tool execution."""
        expected_tools = {
            _AgentAction.DIRECT_ANSWER: [],
            _AgentAction.ITINERARY: [_ToolName.CREATE_ITINERARY],
            _AgentAction.BUDGET: [_ToolName.CALCULATE_BUDGET],
            _AgentAction.ITINERARY_AND_BUDGET: [
                _ToolName.CREATE_ITINERARY,
                _ToolName.CALCULATE_BUDGET,
            ],
        }
        if self.tools_to_use != expected_tools[self.action]:
            raise ValueError(
                "tools_to_use does not match the selected action"
            )
        return self
...
القواعد:
- استخدم الأداتين فقط إذا طلب المستخدم خطة وتوزيع ميزانية معًا بوضوح.
- وجود قيمة budget في preferences وحده لا يعني تشغيل أداة الميزانية.
...
def _local_decision(user_query: str, preferences: _Preferences) -> _AgentDecision:
    ...
    if wants_itinerary and wants_budget:
...
def _get_gemini_model(system_instruction: str) -> Any:
    """Create a Gemini model using the same SDK as vision.py."""
...
_AGENT_DECISION_SCHEMA = {
    ...
    "properties": {
        ...
        "tools_to_use": {
            ...
            "items": {
                "type": "string",
                "enum": ["create_itinerary", "calculate_budget"],
            },
            "maxItems": 2,
        },
        "reason": {"type": "string"},
    },
    "required": ["action", "tools_to_use", "reason"],
}


def _clean_json_text(text: str) -> str:
    """Remove optional Markdown fences from a Gemini JSON response."""
    return (
        text.replace("```json", "")
        .replace("```JSON", "")
        .replace("```", "")
...
def _gemini_decision(
    ...
    model = _get_gemini_model(_AGENT_SYSTEM_PROMPT)
    payload = {
    ...
    return _AgentDecision.model_validate_json(
        _clean_json_text(response.text)
    )
...
def _generate_final_answer(
    ...
    try:
        model = _get_gemini_model(_FINAL_ANSWER_SYSTEM_PROMPT)
        payload = {
            "user_query": user_query,
...
def run_agent(
    ...
    if len(decision.tools_to_use) > MAX_TOOL_STEPS:
        return _error_output(
            "تجاوز الوكيل الحد الأقصى المسموح لخطوات الأدوات.",
            destination=destination,
            error_code="AGENT_STEP_LIMIT",
            warnings=[api_warning] if api_warning else None,
        )
